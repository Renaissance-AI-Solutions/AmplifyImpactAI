...
from app import db
from app.models import PortalUser, ManagedAccount, KnowledgeDocument, ScheduledPost, GeneratedComment, ActionLog
from app.forms import PostComposerForm, SchedulePostForm, CommentSettingsForm
...
def settings():
    ...
    # Pre-populate form with current settings if they exist
    settings = CommentAutomationSetting.query.filter_by(portal_user_id=current_user.id).first()
    if settings:
        comment_settings_form.keywords.data = settings.keywords
        comment_settings_form.monitored_x_accounts.data = settings.monitored_x_accounts
        comment_settings_form.default_posting_account_id.choices = get_managed_account_choices(current_user.id)
        comment_settings_form.default_posting_account_id.data = settings.default_posting_managed_account_id
